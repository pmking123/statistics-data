# metallothionein.csv Codebook

**Filename:** `metallothionein.csv`  
...
Fill in one row per column in `metallothionein.csv`.
...
```r
df <- read.csv("metallothionein.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("metallothionein.csv")
```
