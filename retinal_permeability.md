# retinal_permeability.csv Codebook

**Filename:** `retinal_permeability.csv`  
...
Fill in one row per column in `retinal_permeability.csv`.
...
```r
df <- read.csv("retinal_permeability.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("retinal_permeability.csv")
```
