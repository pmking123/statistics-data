# ukwcs_regression.csv Codebook

**Filename:** `ukwcs_regression.csv`  
...
Fill in one row per column in `ukwcs_regression.csv`.
...
```r
df <- read.csv("ukwcs_regression.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("ukwcs_regression.csv")
```
