# culture-time.csv Codebook

**Filename:** `culture-time.csv`  
...
Fill in one row per column in `culture-time.csv`.
...
```r
df <- read.csv("culture-time.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("culture-time.csv")
```
