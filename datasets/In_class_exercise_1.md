# In_class_exercise_1.csv Codebook

**Filename:** `In_class_exercise_1.csv`  
...
Fill in one row per column in `In_class_exercise_1.csv`.
...
```r
df <- read.csv("In_class_exercise_1.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("In_class_exercise_1.csv")
```
