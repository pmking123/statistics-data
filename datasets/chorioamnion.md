# chorioamnion.csv Codebook

**Filename:** `chorioamnion.csv`  
...
Fill in one row per column in `chorioamnion.csv`.
...
```r
df <- read.csv("chorioamnion.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("chorioamnion.csv")
```
