# leaflet-game.csv Codebook

**Filename:** `leaflet-game.csv`  
...
Fill in one row per column in `leaflet-game.csv`.
...
```r
df <- read.csv("leaflet-game.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("leaflet-game.csv")
```
