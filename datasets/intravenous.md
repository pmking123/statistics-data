# intravenous.csv Codebook

**Filename:** `intravenous.csv`  
...
Fill in one row per column in `intravenous.csv`.
...
```r
df <- read.csv("intravenous.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("intravenous.csv")
```
