# red_cell_folate.csv Codebook

**Filename:** `red_cell_folate.csv`  
...
Fill in one row per column in `red_cell_folate.csv`.
...
```r
df <- read.csv("red_cell_folate.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("red_cell_folate.csv")
```
