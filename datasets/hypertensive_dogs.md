# hypertensive_dogs.csv Codebook

**Filename:** `hypertensive_dogs.csv`  
...
Fill in one row per column in `hypertensive_dogs.csv`.
...
```r
df <- read.csv("hypertensive_dogs.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("hypertensive_dogs.csv")
```
