# toxic_shock.csv Codebook

**Filename:** `toxic_shock.csv`  
...
Fill in one row per column in `toxic_shock.csv`.
...
```r
df <- read.csv("toxic_shock.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("toxic_shock.csv")
```
