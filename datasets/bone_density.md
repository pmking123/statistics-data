# bone_density.csv Codebook

**Filename:** `bone_density.csv`  
...
Fill in one row per column in `bone_density.csv`.
...
```r
df <- read.csv("bone_density.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("bone_density.csv")
```
