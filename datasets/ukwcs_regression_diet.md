# ukwcs_regression_diet.csv Codebook

**Filename:** `ukwcs_regression_diet.csv`  
...
Fill in one row per column in `ukwcs_regression_diet.csv`.
...
```r
df <- read.csv("ukwcs_regression_diet.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("ukwcs_regression_diet.csv")
```
