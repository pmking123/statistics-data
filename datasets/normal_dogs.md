# normal_dogs.csv Codebook

**Filename:** `normal_dogs.csv`  
...
Fill in one row per column in `normal_dogs.csv`.
...
```r
df <- read.csv("normal_dogs.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("normal_dogs.csv")
```
