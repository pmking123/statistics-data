# protein_metabolism.csv Codebook

**Filename:** `protein_metabolism.csv`  
...
Fill in one row per column in `protein_metabolism.csv`.
...
```r
df <- read.csv("protein_metabolism.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("protein_metabolism.csv")
```
