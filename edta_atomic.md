# edta_atomic.csv Codebook

**Filename:** `edta_atomic.csv`  
...
Fill in one row per column in `edta_atomic.csv`.
...
```r
df <- read.csv("edta_atomic.csv", header = TRUE)
```
...
```python
import pandas as pd
df = pd.read_csv("edta_atomic.csv")
```
